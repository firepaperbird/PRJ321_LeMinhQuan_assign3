...
//        HttpServletResponse res = (HttpServletResponse) respnse;
        String uri = req.getRequestURI();
        String url = "login.jsp";
        HttpSession session = ((HttpServletRequest)request).getSession();
        try {
            int lastIndex = uri.lastIndexOf("/");
            String resource = uri.substring(lastIndex+1);//get uri after '/' 
            if(resource.length() > 0){  // is after '/' exist 
                ...
                if (resource.lastIndexOf(".html") > 0 || resource.lastIndexOf(".jsp") > 0) {
                    ...
                    if(resource.contains("index.jsp"))
                        if(session.getAttribute("USERNAME")==null){ // block page if it's not user
                            url="login.jsp";
                        }
                }
                ...
                if(action!=null)
                    switch(action) {
                        case "Register": url = "register.jsp";
                            break;
                        case "profile": url = "profile.jsp";
                            break;
                        case "userCounter": url = "userCounter.jsp";
                            break;
                        case "Logout": url = "LogoutServlet";
                            break;
                    }     
            }
