...
//        String action = request.getParameter("btnAction");
        try {
            ...
            if(su.isUser(userName, password)){ //check use&Pass
                session.setAttribute("USERNAME", userName);
                session.removeAttribute("ERRORID");
                url="index.jsp"; // go to main page
                
                ArrayList<String> lst =(ArrayList<String>) servletContext.getAttribute("LST");
                if(lst==null){
                    lst = new ArrayList<>();
                }
                lst.add(userName); //add user to loged in list
                servletContext.setAttribute("LST",lst);
            }else{
                session.setAttribute("ERRORms","Wrong user/password");
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
